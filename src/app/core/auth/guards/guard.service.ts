...
export const privadoGuardMatch: CanMatchFn = (): MaybeAsync<GuardResult> => {
...
export const guardRoles = (roles: UsuarioRoles[]): CanActivateFn => {
  return () => {
    const rolUsduasrio =
      inject(StorageService).get<ISesionInterface>('sesion')?.usuarioLogin.perfilUsuario;
    if (roles.some((rol) => rol.includes(rolUsduasrio!))) return true;
    else {
