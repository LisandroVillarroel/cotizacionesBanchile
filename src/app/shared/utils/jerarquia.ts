...
export class Jerarquia {
  ...
  // Método para cargar el combo de jerarquías
  //  jerarquiaAnterior(jerarquiaActual:string){
  //   const indice = this.jerarquia.findIndex(j => j.p_codigo_perfil === jerarquiaActual);
  //   if (indice >= 0) { // Verifica que no sea el primer elemento -- indice >= 0
  //     return this.jerarquia[indice - 1]; // Retorna la jerarquía anterior return this.jerarquia[indice - 1];
  //   } else {
  ...
  jerarquiaAnterior(jerarquiaActual: string) {
   ...
   jerarquiaPosterior(jerarquiaActual: string) {
    const indice = this.jerarquia.findIndex(j => j.p_codigo_perfil === jerarquiaActual);
    if (indice >= 0) {
      return this.jerarquia[indice];
    }
    return null;
  }
