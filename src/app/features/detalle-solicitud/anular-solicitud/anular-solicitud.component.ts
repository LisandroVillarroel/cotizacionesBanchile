...
export class AnularSolicitudComponent {
  ...
  anular(motive: string): void {
    ...
    this.anularService.postAnulaSolicitud(this.anulaRequest).subscribe({
      next: (dato) => {
        if (dato.codigo === 200) {
      ...
      error: () => {
        void this.notificacioAlertnService.error('ERROR', 'No fue posible anular la solicitud.');
      },
    });
  }

  getErrorMessage() {
    return this.motivo.hasError('required') ? 'Debe ingresar el motivo de la anulación.' : '';
  }
}
